...
const ContactsListComponent = () => {
  ...
  return (
    <div className="container">
      ...
      <Link className="btn btn-success mb-2" to="/add-contact">
        Add Contact
      </Link>
      ...
      <div>
        <Link
          to={`/contacts?page=${first}`}
          className={`btn btn-info btn-sm ${prev ? "" : "disabled"}`}
        >
          First
        </Link>

        <Link
          to={`/contacts?page=${prev ? prev : first}`}
          className={`btn btn-info btn-sm mx-1 ${prev ? "" : "disabled"}`}
        >
          Previous
        </Link>

        <Link
          to={`/contacts?page=${next ? next : first}`}
          className={`btn btn-info btn-sm ${next ? "" : "disabled"}`}
        >
          Next
        </Link>
      </div>
